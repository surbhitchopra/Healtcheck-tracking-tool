...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        try:
            # Get the customer and process the upload
        ...
        # Handle customer selection (main form submission)
            ...
            if EXCEL_INTEGRATION_AVAILABLE and customer_id.startswith('excel_'):
                try:
                    ...
                    messages.success(request, f'Selected Excel customer: {customer_name}')
...
def api_customer_dashboard_customers(request):
    ...
    print(f"   🔍 Should apply date filtering: {should_filter}")
    ...
    if request.method == 'GET':
        try:
            # EXCEL INTEGRATION: Read from Health_Check_Tracker_1.xlsx
            customers_data = {}
            ...
            try:
                ...
                # Path to the main tracker file
                script_dir = Path(__file__).parent.parent / "Script"
                excel_path = script_dir / "Health_Check_Tracker_1.xlsx"
                ...
                if excel_path.exists():
                    ...
                    # Group data by Network (customer)
                    network_groups = df.groupby('Network')
                    
                    for network_name, group in network_groups:
                        try:
                            # Get basic info from first row of group
                            first_row = group.iloc[0]
                            
                            customer_name = str(network_name).strip()
                            country = str(first_row['Country']) if pd.notna(first_row['Country']) else 'India'
                            node_qty = int(first_row['Node Qty']) if pd.notna(first_row['Node Qty']) else 0
                            gtac_team = str(first_row['GTAC Team']) if pd.notna(first_row['GTAC Team']) else 'PSS'
                            nw_type = str(first_row['NW Type']) if pd.notna(first_row['NW Type']) else '1830 PSS'
                            
                            # Count total runs for this network
                            total_runs = len(group)
                            
                            # Get monthly data from HC Run Date
                            monthly_data = {i: '-' for i in range(1, 13)}  # Initialize all months
                            
                            if 'HC Run Date' in group.columns:
                                for _, row in group.iterrows():
                                    try:
                                        if pd.notna(row['HC Run Date']):
                                            if isinstance(row['HC Run Date'], str):
                                                date_val = pd.to_datetime(row['HC Run Date'])
                                            else:
                                                date_val = row['HC Run Date']
                                            
                                            month = date_val.month
                                            day = date_val.day
                                            year = date_val.year
                                            
                                            # Format: "14-Sep-25" style
                                            month_display = f"{day:02d}-{date_val.strftime('%b')}-{str(year)[-2:]}"
                                            monthly_data[month] = month_display
                                    except:
                                        continue
                            ...
                            last_run_date = 'Never'
                            if 'HC Run Date' in group.columns:
                                valid_dates = group['HC Run Date'].dropna()
                                if not valid_dates.empty:
                                    try:
                                        latest_date = pd.to_datetime(valid_dates).max()
                            ...
                            customers_data[customer_name] = {
                                ...
                                'last_run_date': last_run_date,
                                'networks': [{
                                    'id': hash(customer_name) % 10000,  # Generate a simple ID
                                    'name': customer_name,
                            ...
                            print(f"✅ Processed network: {customer_name} - {country} - {node_qty} nodes - {total_runs} runs")
                ...
                print(f"❌ Excel integration failed: {excel_error}")
                import traceback
                traceback.print_exc()
            
            # Fallback to database if Excel integration failed
            if not customers_data:
                print("🔄 Falling back to database customers...")
                customers_with_networks = Customer.get_customers_with_networks()
                print(f"Found {len(customers_with_networks)} database customers")
            
            for customer_name, networks in customers_with_networks.items():
                try:
                    print(f"\n🔄 Processing {customer_name}: {len(networks)} networks")
                    ...
                    # EXACT same session query as Excel export WITH DATE FILTERING
                    sessions = HealthCheckSession.objects.filter(customer_id__in=network_ids)
                    
                    # Apply date filtering if provided (same as export API)
                    if date_filter:
                        ...
                        print(f"   ⚠️ NO date filter applied to {customer_name}: {sessions.count()} total sessions")
                    ...
                    if date_filter and total_runs == 0:
                        print(f"   🚫 SKIPPING {customer_name}: No sessions found in filtered date range ({date_filter.get('start')} to {date_filter.get('end')})")
                        continue
                    ...
                    try:
                        ...
                        print(f"  ✅ CUSTOMER SUCCESS: {customer_name} -> {customer_country}, {customer_node_count} total nodes")
                    except Exception as helper_error:
                        print(f"  ❌ CUSTOMER HELPER ERROR for {customer_name}: {helper_error}")
                        import traceback
                        traceback.print_exc()
                        
                        # Enhanced inline fallback
                    ...
                    if len(networks) > 1:
                        ...
                        for net in networks:
                            ...
                            # Apply same date filtering to network weight calculation
                            if date_filter:
                                if 'start' in date_filter:
                    ...
                    for i, net in enumerate(networks):
                        ...
                        # Apply same date filtering to individual network runs
                        if date_filter:
                            if 'start' in date_filter:
                                net_sessions = net_sessions.filter(created_at__gte=date_filter['start'])
                            if 'end' in date_filter:
                        ...
                        if len(networks) > 1:
                            network_node_share = int((network_weights[i] / total_sessions_all_nets) * customer_node_count)
                            network_node_share = max(network_node_share, 1)  # At least 1 node per network
                        else:
                            network_node_share = customer_node_count
                        
                        # Each network gets the customer's country (they're all same customer)
                        ...
                        networks_with_runs.append({
                            ...
                            'country': net_country,  # Same as customer country
                            'node_count': network_node_share,  # Distributed node count
                            'location': net_country,  # Alternative field name
                            'node_qty': network_node_share  # Alternative field name
                        })
                        
                        print(f"    → {net_name}: {net_run_count} runs, {network_node_share} nodes, {net_country} (last: {net_last_date})")
                    ...
                    # Store data in format expected by frontend
                    customers_data[customer_name] = {
                        'name': customer_name,
                        'runs': total_runs,
                        'trackers': completed,
                    ...
                    # Still add customer with error info
                    customers_data[customer_name] = {
                        'name': customer_name,
                        'runs': 0,
                        'trackers': 0,
...
def api_export_excel(request):
    ...
    csv_lines.append('Customer,Country,Networks,Node Qty,NE Type,GTAC,Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec,Total Runs')
...
def customer_dashboard_excel(request):
    """
    GET: Reads Script/Health_Check_Tracker_1.xlsx and returns summary data as JSON or Excel download.
    POST: Parses uploaded Excel file.
    """
    from pathlib import Path
    import pandas as pd
    from datetime import datetime
...
def api_customer_dashboard_export(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            writer.writerow(['Customer', 'Country', 'Networks', 'Node Qty', 'NE Type', 'GTAC', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec', 'Total Runs'])
...
@login_required
def debug_customer_data(request):
    """
    Debug endpoint to check what data is available in database
    """
    try:
        from .models import Customer, HealthCheckSession, NodeCoverage
        
        print("\n" + "="*50)
        print("🔍 DEBUG: Checking database content...")
        
        # First check NodeCoverage table
        total_node_coverage = NodeCoverage.objects.all().count()
        print(f"📊 Total NodeCoverage records: {total_node_coverage}")
        
        if total_node_coverage > 0:
            # Show some sample NodeCoverage data
            sample_coverage = NodeCoverage.objects.all()[:5]
            print(f"🔍 Sample NodeCoverage records:")
            for cov in sample_coverage:
                print(f"  - ID: {cov.id}, Node: {cov.node_name}, Customer: {cov.customer_name}, Session: {cov.session_id if hasattr(cov, 'session') else 'N/A'}")
        else:
            print("⚠️ No NodeCoverage records found in database!")
        
        # Check customers
        customers = Customer.objects.filter(is_deleted=False)[:5]
        print(f"\n📊 Found {customers.count()} active customers")
        
        for customer in customers:
            print(f"\n👤 Customer: {customer.name} (ID: {customer.id})")
            
            # Check sessions for this customer
            sessions = HealthCheckSession.objects.filter(customer=customer)
            print(f"   📝 Sessions: {sessions.count()} total")
            
            for session in sessions[:3]:  # Show first 3
                print(f"     - {session.session_id}: {session.status} ({session.created_at.strftime('%Y-%m-%d')})")
                
                # Check NodeCoverage for this session
                nodes = NodeCoverage.objects.filter(session=session)
                print(f"       🖥️ Nodes: {nodes.count()}")
                
                for node in nodes[:3]:  # Show first 3 nodes
                    print(f"         - {node.node_name}: {node.location or 'No location'}")
        ...
        return JsonResponse({
            ...
            'total_sessions': HealthCheckSession.objects.count(),
            'total_nodes': NodeCoverage.objects.count()
        })
