...
def api_customer_dashboard_customers(request):
    ...
    if request.method == 'GET':
        try:
            # EXCEL INTEGRATION DISABLED - Only show DATABASE customers
            customers_data = {}
            
            # SKIP Excel reading - only database customers needed
            print("📊 EXCEL READING DISABLED - Only showing database customers")
            
            # EXCEL INTEGRATION COMPLETELY DISABLED
            print("📊 Excel integration is DISABLED - showing only database customers")
            
            # Skip entire Excel section
            if False:
                    ...
                    for customer_name, group in customer_groups:
                        try:
                            ...
                            customers_data[str(customer_name)] = {
                                ...
                                'monthly_runs': format_monthly_runs_to_array(monthly_runs)_array,
            ...
            for customer_name, networks in customers_with_networks.items():
                try:
                    ...
                    if total_stored_runs > 0 or total_stored_nodes > 0 or has_monthly_data or is_excel_customer:
                        ...
                        # COMBINE both session data and migrated data for accurate count
                        session_runs = sessions.count()
                        total_runs = session_runs + max(total_from_monthly, total_stored_runs)
                        has_migrated_data = True
                        print(f"   ✅ COMBINED DATA: {customer_name} total runs = {total_runs} (sessions: {session_runs} + migrated: {max(total_from_monthly, total_stored_runs)})")
                    else:
                    ...
                    for i, net in enumerate(networks):
                        ...
                        if has_migrated_data:
                            # Calculate runs from monthly data (count non-empty months)
                            if net.monthly_runs and isinstance(net.monthly_runs, dict):
                                net_run_count = sum(1 for v in net.monthly_runs.values() if v and str(v).strip() not in ['-', '', 'null', 'None'])
                                print(f"    📊 Calculated {net.network_name} runs from monthly data: {net_run_count}")
                            else:
                                net_run_count = net.total_runs  # Fallback to stored value
                                print(f"    📊 Using stored {net.network_name} total_runs: {net_run_count}")
                            
                            net_node_share = net.node_qty
                            net_country = net.country if net.country else customer_country
                            # Try to get last run date from monthly_runs JSON or fallback to 'Never'
                            if net.monthly_runs and isinstance(net.monthly_runs, dict):
                                ...
                                net_last_date = 'Never'
                            print(f"    📊 MIGRATED network data for {net.network_name}: {net_run_count} runs, {net_node_share} nodes")
                        else:
                        ...
                        # FIRST: Add fresh session data for this specific network
                        # Filter sessions by network name or ID to match exact network
                        net_sessions = HealthCheckSession.objects.filter(customer=net)
                        
                        # ADDITIONAL FILTER: Try to match by network name if available
                        if hasattr(net, 'network_name') and net.network_name and 'East_Zone_DWDM' in net.network_name:
                            # This is East DWDM network - get its specific sessions
                            print(f"    🎯 TARGETING East DWDM network: {net.network_name}")
                        elif hasattr(net, 'network_name') and net.network_name and 'East_Zone_OTN' in net.network_name:
                            # This is East OTN network - get its specific sessions  
                            print(f"    🎯 TARGETING East OTN network: {net.network_name}")
                        
                        print(f"    🔍 Network {net_name} (ID: {net.id}) has {net_sessions.count()} sessions")
                        
                        # Apply same date filtering to network sessions
                        if date_filter:
                            if 'start' in date_filter:
                                net_sessions = net_sessions.filter(created_at__gte=date_filter['start'])
                            if 'end' in date_filter:
                        ...
                        # Add fresh network sessions to monthly array - GET LATEST DATE
                        if net_sessions.exists():
                            print(f"    🔄 Adding {net_sessions.count()} FRESH sessions for network {net_name}...")
                            # Get the LATEST session for each month
                            for month_num in range(1, 13):
                                month_sessions = net_sessions.filter(created_at__month=month_num)
                        ...
                        networks_with_runs.append({
                            ...
                            'monthly_runs': net_monthly_array,  # ARRAY format for frontend
                    ...
                    # FIRST: Add fresh session data to monthly array - GET LATEST DATE PER MONTH
                    if sessions.exists():
                    ...
                    customers_data[final_key] = {
                        ...
                        'monthly_runs': customer_monthly_runs_array,  # ARRAY format for frontend table
